...
select 
  ...
  count(*) as claims,
  sum(claim_line_paid_amt) as spend 
from prod_a4806_fe.insights.activity  a
left join (select * from prod_a4806_fe.insights.patient_x_month where attribution_type = 'as_is') pxm
  on pxm.fk_patient_id = a.fk_patient_id
  and a.activity_from_month_cd = pxm.month_cd
where 
  claim_type_cd = '40' and 
...
union ALL
select * from a4806_2021_therapy)
...
left join prod_common_fe.od.od_ccn_prvdr_service_reg_201609 ccn
  on ccn.prvdr_num = combo.facility_ccn_num
group by 1,2
order by 5 desc 